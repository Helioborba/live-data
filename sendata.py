...
def getAtivo():
    return choices(ativosArray, weights=[index * value * 30 if value == 34 else index * value  for index,value in enumerate(ativosArray)], k=1)[0]
...
def criaData():
    ...
    # [index * value * 30 if value == 34 else index * value  for index,value in enumerate(numba)]
    date_string = f"2022-{choices(numbers, weights=[index * value * 30 if value == 10 or value == 4 else index for index,value in enumerate( numbers)], k=1)[0]}-{randrange(1,30)}"
...
while True:
    ...
    r = requests.post('http://localhost:5000/client_php/model/adicionar.php',data=obj.postRandomData())
    listr = r.json()
    print(f"A resposta do servidor foi: {listr['MSG']}")
    objectArray.append(obj)